...
def accumulate(merger, start, n, term):
    ...
    list1 = [i for i in range(1, n+1)]
    list2 = [start] + list(map(term, list1))
    result = my_reduce(merger, list2)
    return result
...
def filtered_accumulate(merger, start, cond, n, term):
    ...
    def merge_if(x, y):
        ...
        if cond(y):
            ...
            return x  # x represents all the things we have successfully merged so far.
...
def funception(func_a, start):
    ...
    def func_b(n):
        ...
        if n < start:
            return func_b(start + 1)
        else:
            list1 = list(range(start, n))
            # print(list1)
            list2 = list(map(func_a, list1))
            return my_reduce(mul, list2)
    return func_b