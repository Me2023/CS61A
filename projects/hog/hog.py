...
def oink_points(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    ss = max(2 * (opponent_score // 10 - (opponent_score // 100 * 10)) - opponent_score % 10, 1)
    return ss
    # END PROBLEM 2
...
def pigs_on_prime(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    n = player_score + 1
    if player_score == 1 or is_prime(player_score):
        while (not is_prime(n)) and (not n == 1):
            n += 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    pl = None  # To be used in problem 7
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    while score0 < goal and score1 < goal:
        if who == 0:
            score0 += take_turn(strategy0(score0, score1), score0, score1, dice, goal=GOAL_SCORE)
            score0 += pigs_on_prime(score0, score1)

        elif who == 1:
            score1 += take_turn(strategy1(score1, score0), score1, score0, dice, goal=GOAL_SCORE) 
            score1 += pigs_on_prime(score1, score0)

        pl, ms = say(score0, score1, pl)
        if ms != None:
            print(ms)
        who = next_player(who)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if last_leader != 0 and score0 > score1:
        ...
        return 0, message
    elif last_leader != 1 and score1 > score0:
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    number, average_turn_score = 1, 0
    maxscore, times = 0, 0
    while number <= 10:
        average_turn_score = make_averaged(roll_dice, total_samples)(number, dice)
        if average_turn_score > maxscore:
            maxscore = average_turn_score 
            times = number
        number += 1
    return times
    # END PROBLEM 9
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if oink_points(score, opponent_score) >= threshold or is_prime(score + oink_points(score, opponent_score)):
        return 0
    else:
        return num_rolls 
    # END PROBLEM 11
